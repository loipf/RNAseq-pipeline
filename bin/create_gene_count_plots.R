...
pacman::p_load(yaml, knitr, data.table, plotly, ggplot2, pheatmap, vsn, hexbin, LSD, cowplot)
...
### read in everything
reads_qc_table =  data.frame(fread(params$READS_QC_TABLE_FILE, header = T), stringsAsFactors = FALSE, row.names = 1)
gene_matrix =  data.frame(fread(params$GENE_MATRIX_FILE, header = T), stringsAsFactors = FALSE, row.names = 1)  ### genes x samples
gene_matrix_vst =  data.frame(fread(params$GENE_MATRIX_FILE_VST, header = T), stringsAsFactors = FALSE, row.names = 1)
